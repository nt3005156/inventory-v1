...
describe('supplier statement period validation and deterministic ledger', () => {
  ...
  it('orders same-time invoices before payments and preserves balances across pages', async () => {
    const first = await createInvoice({invoiceNo: 'SAME-1', invoiceDate: '2026-08-01', subtotal: 100});
    const second = await createInvoice({invoiceNo: 'SAME-2', invoiceDate: '2026-08-01', subtotal: 200});
    assert.equal(first.status, 201, first.body?.message);
    assert.equal(second.status, 201, second.body?.message);
    assert.equal((await postPayment(first.body, {amount: 13, paidAt: '2026-08-01'})).status, 201);
    assert.equal((await postPayment(second.body, {amount: 26, paidAt: '2026-08-01'})).status, 201);

    const firstPage = await statement({from: '2026-08-01', to: '2026-08-01', page: '1', limit: '2'});
    const secondPage = await statement({from: '2026-08-01', to: '2026-08-01', page: '2', limit: '2'});
  ...
  it('keeps the pure ledger builder deterministic without database state', () => {
    const invoices = [
      {_id: 'b', invoiceNo: 'B', invoiceDate: new Date('2026-07-31T18:15:00.000Z'), total: 20, status: 'unpaid', createdAt: new Date('2026-08-01T00:00:02Z')},
      {_id: 'a', invoiceNo: 'A', invoiceDate: new Date('2026-07-31T18:15:00.000Z'), total: 10, status: 'unpaid', createdAt: new Date('2026-08-01T00:00:01Z')}
    ];
    const payments = [{
      _id: 'p', paymentNo: 'PAY', invoice: {_id: 'a', invoiceNo: 'A'}, paidAt: new Date('2026-07-31T18:15:00.000Z'),
      amount: 5, method: 'cash', status: 'posted', createdAt: new Date('2026-08-01T00:00:00Z')
    }];
    const result = buildStatementLines(invoices, payments, {period: statementPeriod({from: '2026-08-01', to: '2026-08-01'})});
...
describe('supplier statement balances and aging', () => {
  it('separates opening balance, period activity, closing balance, and aging buckets', async () => {
    const opening = await createInvoice({invoiceNo: 'OPENING', invoiceDate: '2026-06-01', dueDate: '2026-06-15', subtotal: 100});
    const current = await createInvoice({invoiceNo: 'JULY', invoiceDate: '2026-07-10', dueDate: '2026-07-15', subtotal: 200});
    assert.equal(opening.status, 201, opening.body?.message);
    assert.equal(current.status, 201, current.body?.message);
    assert.equal((await postPayment(current.body, {amount: 50, paidAt: '2026-07-20'})).status, 201);

    const response = await statement({from: '2026-07-01', to: '2026-07-31'});
    assert.equal(response.status, 200, response.body?.message);
    assert.deepEqual(response.body.period, {from: '2026-07-01', to: '2026-07-31', asOf: '2026-07-31', timezone: 'Asia/Kathmandu'});
  ...
  it('reconstructs payment and reversal state at the requested as-of date', async () => {
    const invoice = await createInvoice({invoiceNo: 'HISTORICAL', invoiceDate: '2026-08-10', dueDate: '2026-08-12', subtotal: 100});
    const paid = await postPayment(invoice.body, {amount: 40, paidAt: '2026-08-12'});
    ...
    const beforeReversal = await statement({to: '2026-08-15'});
    ...
    const afterReversal = await statement({to: '2026-08-16'});
    ...
    const balance = await request(`/api/suppliers/${supplier._id}/balance?branch=${world.branchA._id}&asOf=2026-08-15`, {token: tokenFor(world.owner)});
    assert.equal(balance.status, 200, balance.body?.message);
    assert.equal(balance.body.asOf, '2026-08-15');
  ...
  it('includes an invoice before its void timestamp and excludes it afterward', async () => {
    const created = await createInvoice({invoiceNo: 'VOID-HISTORY', invoiceDate: '2026-08-10', subtotal: 100});
    ...
    const historical = await statement({to: '2026-08-15'});
    ...
    const current = await statement({to: '2026-08-16'});
    ...
    const voidPeriod = await statement({from: '2026-08-16', to: '2026-08-16'});
