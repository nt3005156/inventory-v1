...
export async function moveStock({branch,ingredient,qty,type,reason,referenceType,referenceId,user,idempotencyKey,unit,unitCost=0},session){
  ...
  if(after<=balance.reorderLevel){const ing=await Ingredient.findById(ingredient).session(session||null);const name=ing?.name||'Ingredient';await Notification.create([{branch,type:after<=0?'out_of_stock':'low_stock',title:after<=0?'Out of stock':'Low stock',body:`${name} is ${after<=0?'out of stock':'at reorder level'}`,referenceId:ingredient}],{session});}
  return tx[0];
}
