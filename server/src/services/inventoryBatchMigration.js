...
import {Branch, InventoryBalance, InventoryBatch, Order} from '../models/operations.js';
  ...
  try {
    await session.withTransaction(async () => {
      ...
      if (!balances.length) {
        ...
        result = {backfilled: 0, unresolved: 0, singletonFieldsRemoved: removed.modifiedCount};
        return;
      }
      ...
      const [branches, ingredients, tracked] = await Promise.all([
        Branch.find({_id: {$in: branchIds}}).select('_id restaurant').session(session).lean(),
        Ingredient.find({_id: {$in: ingredientIds}}).select('_id restaurant unit').session(session).lean(),
      ...
      const ingredientById = new Map(ingredients.map(item => [String(item._id), item]));
      ...
      for (const balance of balances) {
        ...
        const ingredient = ingredientById.get(String(balance.ingredient || ''));
        if (!restaurant || !ingredient || String(ingredient.restaurant || '') !== String(restaurant)) {
          unresolved += 1;
          continue;
        }
        ...
        if (existingLegacy) {
          throw new Error(`Batch migration will not silently rewrite lot ${existingLegacy._id}; its quantity does not match the immutable inventory ledger balance`);
        }
      ...
      // Validate the complete legacy state before issuing the first write. The transaction
      // then guarantees an unexpected write failure cannot leave a partial backfill.
      if (unresolved) {
        throw new Error(`Inventory batch migration found ${unresolved} balance record(s) without valid tenant ownership`);
      }
      if (planned.length) await InventoryBatch.create(planned, {session, inventoryLedgerWrite: true});
      const removed = await InventoryBalance.collection.updateMany(
        {$or: [{batchNumber: {$exists: true}}, {expiryDate: {$exists: true}}]},
        {$unset: {batchNumber: '', expiryDate: ''}},
        {session}
      );
      result = {backfilled: planned.length, unresolved: 0, singletonFieldsRemoved: removed.modifiedCount};
    });
    return result;
  } finally {
...
export async function ensureInventoryBatchIndexes() {
  ...
  return {...backfill, indexes: [...BATCH_INDEXES.map(index => index.options.name), orderSourceIndex]};
}
