...
import {User,Ingredient,MenuItem,Expense,Audit} from './models/index.js';import {auth} from './middleware/auth.js';import {audit} from './services/engine.js';import http from 'http';import operations from './routes/operations.js';import supplierCatalog from './routes/supplierCatalog.js';import {attachRealtime,closeRealtime} from './services/realtime.js';import {configuredClientOrigins,ensureOperationalIndexes,validateRuntimeEnvironment,verifyTransactionCapableDatabase} from './services/startup.js';
...
const ingredientInventoryFields=['stockQty','averageCost','quantity','unitCost'];
const hasIngredientInventoryFields=body=>ingredientInventoryFields.some(field=>Object.hasOwn(body||{},field));
app.get('/api/ingredients',auth(),async(req,res)=>{const user=await User.findById(req.user.id).select('restaurantId');if(!user?.restaurantId)return res.status(403).json({message:'User restaurant is unavailable'});res.json(await Ingredient.find({restaurant:user.restaurantId}).sort({name:1}).populate('supplier'))});
app.post('/api/ingredients',auth(['owner','manager']),async(req,res)=>{if(hasIngredientInventoryFields(req.body))return res.status(400).json({message:'Ingredient stock and cost can only be changed through the inventory ledger'});const user=await User.findById(req.user.id).select('restaurantId');if(!user?.restaurantId)return res.status(403).json({message:'User restaurant is unavailable'});const row=await Ingredient.create({...req.body,restaurant:user.restaurantId});await audit({entity:'ingredients',entityId:row._id,restaurant:user.restaurantId,action:'create',after:row,user:req.user.id});res.status(201).json(row)});
app.patch('/api/ingredients/:id',auth(['owner','manager']),async(req,res)=>{if(hasIngredientInventoryFields(req.body))return res.status(400).json({message:'Ingredient stock and cost can only be changed through the inventory ledger'});const user=await User.findById(req.user.id).select('restaurantId');const before=await Ingredient.findOne({_id:req.params.id,restaurant:user?.restaurantId});if(!before)return res.status(404).json({message:'Ingredient not found'});const changes={...req.body};delete changes.restaurant;const row=await Ingredient.findOneAndUpdate({_id:before._id,restaurant:user.restaurantId},changes,{new:true,runValidators:true});await audit({entity:'ingredients',entityId:row._id,restaurant:user.restaurantId,action:'update',before,after:row,user:req.user.id});res.json(row)});
